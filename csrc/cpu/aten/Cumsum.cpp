...
at::Tensor& cumsum(
  ...
  // at::Tensor result = at::empty_like(casted_self,
  ...
  std::cout << "enter cumsum func" << std::endl;
  return cumsum_kernel_stub(kCPU, activation, weight, output, m, n, k);
}
...
//     c10::optional<at::ScalarType> dtype,
//     at::Tensor& result) {
//   // cumsum_kernel_impl(result, self.toType(result.scalar_type()), dim,
//   dtype); cumsum_kernel_stub(
...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  m.def(
      "cumsum(Tensor activation, Tensor weight, Tensor(a!) output, int m, int n, int k) -> Tensor(a!)");
